...
PLAN UPDATE, as of 14 March 2026

We are going to hyper-focus this early version of Minai specifically on being a great assistant to human virtual assistants. The idea is to empower virtual assistants from countries like Nigeria and the Philippines to accomplish their tasks more easily and more accurately, while reducing overall time spent. We want our language support to include English, Swahili, and Filipino.

As part of this hyper-focus, we want to do two critical product changes:

1. Every message thread now lives inside a notebook. You can only have one message thread per notebook, but you can have an unlimited number of notes. You can create a note inside a notebook in two ways. The first is by selecting a message in the message thread and creating a new note with it. The second is by opening up the active notebook and adding a new note to it. The sidebar will be where we do all of this notebook management, and it will now have three widths on the desktop viewport: the collapsed/hidden one, the 20% width (standard sidebar width), and the expanded width that fills up nearly the entire viewport. On mobile, we will only have the collapsed and the expanded widths. Part of the onboarding process should explain to the user that each notebook is intended to represent one distinct client or project, and they should name them accordingly. Each notebook will probably need its own settings menu. Initially it will only have a name and timezone, because we assume that the user's timezone is likely different from the timezone of the client.

2. After arriving in the chat interface, the app will offer the user to login with their Google account and give full access to their Google calendar. This can also be done in the general Settings menu (also in the sidebar) at a later time and is not required. This level of calendar access is important because we will need to be able to look at all the calendars that are shared by their clients, in order to present free/busy timeslots. Whenever we create a new Google calendar event for them, we will present its details in the chat along with a link so they can manipulate it further in the actual Google calendar interface. The calendar access must be intelligent enough to understand if a shared calendar belongs to Client A or Client B, depending on what notebooks have been created. Whenever a calendar event is created, it is done so in the timezone of the client, not the timezone of the user.