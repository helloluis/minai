...
const MODEL_FAST: ModelId = "qwen3.6-flash";
...
type ClassifyResult = {
  classification: "simple" | "balanced" | "deep";
...
async function classifyWithDashscope(
  userMessage: string,
  recentContext?: string,
): Promise<ClassifyResult & { latencyMs: number }> {
  const t0 = Date.now();
...
async function classifyWithOllama(
  ...
  try {
    ...
    const classification = raw.includes("chitchat")
      ? ("simple" as const)
      : ("balanced" as const);
    const latencyMs = Date.now() - t0;
    ...
    const latencyMs = Date.now() - t0;
    console.error(
      "[Router:ollama] Classification failed, falling back to dashscope:",
      err,
    );
...
async function classifyPrompt(
  ...
  if (CLASSIFIER_COMPARE) {
    const [primary, secondary] =
      CLASSIFIER_PROVIDER === "ollama"
        ? await Promise.all([
            classifyWithOllama(userMessage, recentContext),
            classifyWithDashscope(userMessage, recentContext),
          ])
        : await Promise.all([
            classifyWithDashscope(userMessage, recentContext),
            classifyWithOllama(userMessage, recentContext),
    ...
    const primaryName =
      CLASSIFIER_PROVIDER === "ollama" ? "ollama" : "dashscope";
    const secondaryName =
      CLASSIFIER_PROVIDER === "ollama" ? "dashscope" : "ollama";
  ...
  if (CLASSIFIER_PROVIDER === "ollama") {
    ...
    console.log(
      `[Classifier:ollama] "${preview}" → ${result.classification} (${result.latencyMs}ms)`,
    );
  ...
  console.log(
    `[Classifier:dashscope] "${preview}" → ${result.classification} (${result.latencyMs}ms)`,
  );
  return { classification: result.classification, usage: result.usage };
}
 ...
 */
function findContextImages(
  chatHistory: { role: string; images?: string[] | null; content: string }[],
): ContextImage[] {
  ...
  for (let i = chatHistory.length - 1; i >= start; i--) {
    ...
    if (msg.role === "assistant" && msg.content) {
      const imgMatches = msg.content.matchAll(
        /!\[[^\]]*\]\((https?:\/\/[^)]+\/api\/uploads\/[^)]+)\)/g,
      );
  ...
  console.log(
    `[Router] findContextImages: found ${results.length} image(s) — ${results.filter((i) => i.source === "user").length} user, ${results.filter((i) => i.source === "generated").length} generated`,
  );
  return results;
}
...
async function buildMessages(
  ...
  const calProviders = [
    hasGoogle ? "Google" : "",
    hasMicrosoft ? "Microsoft Teams" : "",
  ].filter(Boolean);
  const calStatus =
    calProviders.length > 0
  ...
  if (!images?.length && contextImages.length > 0) {
    const listing = contextImages
      .map((img, i) => `  [${i}] ${img.label} (${img.source})`)
      .join("\n");
    messages.push({
      role: "system",
...
export async function* streamResponse(
  ...
  // Classifier retired 2026-05-11 — auto mode maps directly to deep
  let model: ModelId;
  ...
  if (mode === "fast" || mode === "balanced") {
    ...
    // Auto mode — retired, map directly to deep
    model = MODEL_DEEP;
    enableThinking = true;
    classification = "deep";
    console.log(`[Router] Auto mode retired — mapping to deep`);
  }
  ...
  // Build messages
  const { messages, contextImages } = await buildMessages(
    conversationId,
    userId,
    userMessage,
    images,
  ...
  const imageContext = images?.length
    ? images.map((url) => ({
        ...
        label: "Current message upload",
      }))
    : contextImages;
  console.log(
    `[Router] imageContext: ${imageContext.length} image(s)${imageContext.length > 0 ? `, first=${imageContext[0].url.slice(0, 60)}` : ""}`,
  );

  if (images && images.length > 0) {
    console.log(
      `[Router] Sending ${images.length} image(s) to ${model}, first image size: ${images[0].length} chars`,
    );
  }
  ...
  while (iterations < MAX_TOOL_ITERATIONS) {
    ...
    console.log(
      `[Router] Tool loop iteration ${iterations}: ${result.toolCalls.map((t) => t.name).join(", ")}`,
    );
    ...
    for (const tc of result.toolCalls) {
      try {
        ...
        if (toolResult.cost_usd) {
          if (totalUsage) {
            ...
            totalUsage = {
              inputTokens: 0,
              outputTokens: 0,
              cost: toolResult.cost_usd,
            };
          }
          console.log(
            `[Router] Tool ${tc.name} added cost: $${toolResult.cost_usd}`,
          );
        }
        ...
        try {
          ...
          /* not JSON or no action directives */
        }
      } catch (err) {
    ...
    console.log(
      `[Router] Continuing with ${result.toolCalls.length} tool result(s) injected`,
    );
  }

  if (iterations >= MAX_TOOL_ITERATIONS) {
    console.warn(
      `[Router] Tool loop hit max iterations (${MAX_TOOL_ITERATIONS})`,
    );
  }
  ...
  if (totalUsage) {
    ...
    console.log(
      `[Router] Cost: $${cost.toFixed(6)}, freeCreditUsed: $${freeCreditUsed.toFixed(6)}, chargeable: $${chargeableCost.toFixed(6)}`,
    );
    ...
    console.log(
      `[Router] Updated balance: free_credit=$${updatedBalance?.free_credit_usd}, paid=$${updatedBalance?.balance_usd}`,
    );
