...
const MODES: {
  ...
  },
  {
    value: "fast",
    label: "Fast",
    description: "Qwen Flash — quick and cheap",
    icon: FlashIcon,
  },
  {
    value: "balanced",
    label: "Balanced",
    description: "Qwen Plus — capable, no deep reasoning",
    icon: BalancedIcon,
  },
  {
    value: "deep",
    label: "Deep",
    description: "Qwen Plus — extended reasoning",
    icon: DeepIcon,
  },
];
...
export function ChatInput() {
    ...
    (files: FileList | File[]) => {
      ...
      for (const file of files) {
        ...
        if (IMAGE_TYPES.has(file.type)) {
          ...
          DOC_TYPES.has(file.type) ||
          file.name.match(/\.(pdf|docx?|txt|csv|md)$/i)
        ) {
          ...
          uploadFile(activeConversationId, file)
            .then((uploaded) => {
              setStagedDocs((prev) =>
                prev.map((d) =>
                  d.name === docName
                    ? { ...d, uploading: false, fileId: uploaded.id }
                    : d,
                ),
              );
            })
            .catch((err) => {
              setStagedDocs((prev) =>
                prev.map((d) =>
                  d.name === docName
                    ? { ...d, uploading: false, error: err.message }
                    : d,
                ),
              );
            });
  ...
  const handleSubmit = async () => {
    ...
    if ((!trimmed && stagedImages.length === 0 && !hasFiles) || isStreaming)
      return;
    ...
    const message =
      trimmed ||
      (images ? "What is this image?" : "Please review the attached file.");
      ...
      <div className="max-w-3xl mx-auto relative">
        ...
        {isDragging && (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-xl border-2 border-dashed border-minai-500 bg-minai-50/80 dark:bg-minai-900/30 pointer-events-none">
            <span className="text-sm font-medium text-minai-600">
              Drop file here
            </span>
          </div>
        )}

        {/* Mode selector — classifier retired 2026-05-11, only Deep is active */}
        <div className="flex gap-1 mb-2">
          {MODES.map((m) => {
            const isActive = m.value === "deep";
            const isSelected = mode === m.value;
            return (
              <button
                key={m.value}
                onClick={() => isActive && setMode(m.value)}
                title={
                  isActive ? m.description : "Disabled — classifier retired"
                }
                ...
                disabled={!isActive}
              >
                <m.icon size={12} disabled={!isActive} />
                {m.label}
              </button>
            );
          })}
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            placeholder={
              stagedImages.length > 0
                ? "Describe the image, or send as-is..."
                : "Type a message..."
            }
            rows={1}
            disabled={isStreaming}
          ...
          <button
            ...
            disabled={
              (!text.trim() && stagedImages.length === 0) || isStreaming
            }
